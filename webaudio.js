export default class WebAudio {
    constructor(url) {
        ...
        this.maximumAudioLag = 1.0; // 1.5, amount of seconds we can potentially be behind the server audio stream
        this.syncLagInterval = 4000; // 5000, check every x milliseconds if we are behind the server audio stream
        this.updateBufferEvery = 20; // 20, add recieved data to the player buffer every x milliseconds
        this.reduceBufferInterval = 500; // 500, trim the output audio stream buffer every x milliseconds so we don't overflow
        this.maximumSecondsOfBuffering = 1; // 1, maximum amount of data to store in the play buffer
        this.connectionCheckInterval = 500; // 500, check the connection every x milliseconds
